...
def _resolve(file: File, page: Page):
    ...
    return (
        os.path.extsep.join(norm.split(os.path.extsep))
        if os.path.extsep in norm
        else norm
    )
...
class Badges:
    ...
    def get_badge_for_version(text: str, page: Page, files: Files):
        ...
        return Badges.create_badge(
            ...
            text=f"[{text}]({_resolve_path(path, page, files)}){'{ data-preview  }'}"
            if spec
            else "",
        )
    ...
    def get_badge_for_utility(text: str, page: Page, files: Files):
        ...
        return Badges.create_badge(
            icon=f"[:{icon}:]({href} 'Third-party utility')", text=text
        )
    ...
    def get_badge_for_example_view_and_download(text: str) -> str:
        ...
        def _get_badge_for_example_view() -> str:
            ...
            return Badges.create_badge(
                icon=f"[:{icon}:]({href} 'View example')", type="right"
            )

        return "\n".join(
            [_get_badge_for_example_download(), _get_badge_for_example_view()]
        )
    ...
    def get_badge_for_demo_repository(text: str):
        ...
        return Badges.create_badge(
            icon=f"[:{icon}:]({href} 'Demo repository')", text=text, type="right"
        )
    ...
    def get_badge_for_experimental_feature(page: Page, files: Files):
        ...
        return Badges.create_badge(icon=f"[:{icon}:]({href} 'Experimental')")
