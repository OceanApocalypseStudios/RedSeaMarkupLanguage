...
namespace RSML.Semantics
{
	...
	public sealed class RsValidator : IValidator
	{
		...
		/// <inheritdoc />
		public ImmutableHashSet<string> ValidArchitectures => [ "==", "!=", "<", ">", "<=", ">=" ];
		...
		public void ValidateLine(ReadOnlySpan<RsToken> tokens)
		{
			...
			var actualTokens = tokens.Length == 1 ? tokens : (tokens[ ^1 ].Type == RsTokenType.Eol ? tokens[ ..^1 ] : tokens);
			...
			#region Logic Path
			{
				...
				switch (actualTokens.Length)
				{
					...
					case 3:
						...
						var sysName1 = actualTokens[ 1 ].Value;
						...
						if ((!ValidSystems.Any(s => sysName1.Equals(s, StringComparison.OrdinalIgnoreCase))) && actualTokens[ 1 ].Type == RsTokenType.SystemName)
					...
					case 4:
						...
						var sysName2 = actualTokens[ 1 ].Value;
						var archName1 = actualTokens[ 2 ].Value;
						...
						if ((!ValidSystems.Any(s => sysName2.Equals(s, StringComparison.OrdinalIgnoreCase))) && actualTokens[ 1 ].Type == RsTokenType.SystemName)
							throw new InvalidRsmlSyntax("Invalid system name as of v2.0.0.");

						if ((!ValidArchitectures.Any(s => archName1.Equals(s, StringComparison.OrdinalIgnoreCase))) && actualTokens[ 2 ].Type == RsTokenType.ArchitectureIdentifier)
					...
					case 5:
						if ((actualTokens[ 1 ].Type != RsTokenType.SystemName &&
							...
							throw new InvalidRsmlSyntax("A 5 token long logic path must be a *Operator + SystemName + MajorVersionId + ArchitectureIdentifier + LogicPathValue overload.");

						var sysName3 = actualTokens[ 1 ].Value;
						var major1 = actualTokens[ 2 ].Value;
						var archName2 = actualTokens[ 3 ].Value;
						...
						if ((!ValidSystems.Any(s => sysName3.Equals(s, StringComparison.OrdinalIgnoreCase))) && actualTokens[ 1 ].Type == RsTokenType.SystemName)
							throw new InvalidRsmlSyntax("Invalid system name as of v2.0.0.");

						if ((!ValidArchitectures.Any(s => archName2.Equals(s, StringComparison.OrdinalIgnoreCase))) && actualTokens[ 3 ].Type == RsTokenType.ArchitectureIdentifier)
							throw new InvalidRsmlSyntax("Invalid architecture identifier as of v2.0.0.");
					...
					case 6:
						if ((actualTokens[ 1 ].Type != RsTokenType.SystemName &&
							...
							throw new InvalidRsmlSyntax("A 5 token long logic path must be a *Operator + SystemName + |Comparator| + MajorVersionId + ArchitectureIdentifier + LogicPathValue overload.");

						var sysName4 = actualTokens[ 1 ].Value;
						var comp = actualTokens[ 2 ].Value;
						var major2 = actualTokens[ 3 ].Value;
						var archName3 = actualTokens[ 4 ].Value;
						...
						if ((!ValidSystems.Any(s => sysName4.Equals(s, StringComparison.OrdinalIgnoreCase))) && actualTokens[ 1 ].Type == RsTokenType.SystemName)
							throw new InvalidRsmlSyntax("Invalid system name as of v2.0.0.");

						if ((!ValidArchitectures.Any(s => archName3.Equals(s, StringComparison.OrdinalIgnoreCase))) && actualTokens[ 1 ].Type == RsTokenType.ArchitectureIdentifier)
							throw new InvalidRsmlSyntax("Invalid architecture identifier as of v2.0.0.");

						if (!ValidComparators.Any(s => comp.Equals(s, StringComparison.OrdinalIgnoreCase)))
							throw new InvalidRsmlSyntax("Invalid comparator.");

						if ((!Int32.TryParse(major2, out _)) && actualTokens[ 1 ].Type == RsTokenType.MajorVersionId)
							throw new InvalidRsmlSyntax("The major version must be a valid integer");
		...
		public void ValidateBuffer(ReadOnlySpan<RsToken> bufferTokens)
		{
			...
			while (pos < bufferTokens.Length)
			{
				...
				var eofHit = SkipNewlines(bufferTokens, ref pos);
