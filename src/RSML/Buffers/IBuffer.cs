...
namespace OceanApocalypseStudios.RSML.Buffers
{
	...
	public partial interface IBuffer<TItem> : IDisposable
	{
		...
		/// <summary>
		/// Returns the index of the next whitespace item in the buffer, relative to the cursor position.
		/// Line separators are included in the whitespace category.
		/// </summary>
		/// <returns>The index of the next whitespace item, relative to the cursor position.</returns>
		int GetNextWhitespaceIndex();

		/// <summary>
		/// Returns the index of the next line separator in the buffer, relative to the cursor position.
		...
		int GetNextLineSeparatorIndex();

		/// <summary>
		/// Returns the index of the next whitespace item in the buffer, relative to a given <paramref name="index"/>.
		/// Line separators are included in the whitespace category.
		/// </summary>
		/// <returns>The index of the next whitespace item, relative to a <paramref name="index"/>.</returns>
		int GetNextWhitespaceIndex(int index);

		/// <summary>
		/// Returns the index of the next line separator in the buffer, relative to a given <paramref name="index"/>.
		/// Only line separators count - regular whitespace do not.
		/// </summary>
		/// <returns>The index of the next line separator, relative to an <paramref name="index"/>.</returns>
		int GetNextLineSeparatorIndex(int index);
		...
		bool TryReadLine(ReadOnlySpan<TItem> line, out int consumed);
		...
		/// <param name="consumed">The amount of items that were consumed.</param>
		/// <returns>False if the buffer was fully consumed, there are no more lines to read or an exception occured.</returns>
		bool TryReadLine(int index, ReadOnlySpan<TItem> line, out int consumed);
		...
		/// </remarks>
		/// <returns>False if the buffer was fully consumed or an exception occured.</returns>
		bool TryReadWord(ReadOnlySpan<TItem> destination, out bool isWhitespace, out int consumed);
		...
		/// </remarks>
		/// <returns>False if the buffer was fully consumed or an exception occured.</returns>
		bool TryReadWord(int index, ReadOnlySpan<TItem> destination, out bool isWhitespace, out int consumed);
		...
		/// </remarks>
		/// <returns>False if the buffer was fully consumed or an exception occured.</returns>
		bool TryReadWord(TItem itemKind, ReadOnlySpan<TItem> destination, out bool isItemKind, out int consumed);
		...
		/// will be set to something greater than <c>0</c>.
		/// </remarks>
		/// <returns>False if the buffer was fully consumed, the reading was only partial or an exception occured.</returns>
		bool TryReadWord(int index, TItem itemKind, ReadOnlySpan<TItem> destination, out bool isItemKind, out int consumed);
		...
		/// <returns>False if the buffer was fully consumed or an exception occured.</returns>
		bool TryReadUntilWhitespace(ReadOnlySpan<TItem> destination, out int consumed);
		...
		/// <param name="consumed">The amount of consumed items.</param>
		/// <returns>False if the buffer was fully consumed or an exception occured.</returns>
		bool TryReadUntilWhitespace(int index, ReadOnlySpan<TItem> destination, out int consumed);
		...
		/// </returns>
		int ReadWhile(Func<int, TItem, bool> predicate, ReadOnlySpan<TItem> destination);
		...
		/// The amount of read (consumed) items; <c>-1</c> if no items were consumed or an error occured (in which case <paramref name="destination"/> will be empty).
		/// </returns>
		int ReadWhile(int index, Func<int, TItem, bool> predicate, ReadOnlySpan<TItem> destination);
		...
		/// <summary>
		/// Consumes from the buffer while a <paramref name="predicate"/> returns true.
		/// Starts at <paramref name="index"/>. Does not save items.
		/// </summary>
		/// <param name="index">The index at which to start skipping.</param>
		/// <param name="predicate">
		/// A function that takes the current index (relative to <paramref name="index"/>),
		/// which is incremented every item, and the item associated with it. Execution stops when
		/// the predicate returns <c>false</c> or the buffer is fully consumed.
		...
		/// The amount of consumed items; <c>-1</c> if no items were consumed or an error occured.
		/// </returns>
		int SkipWhile(int index, Func<int, TItem, bool> predicate);
		...
		/// <summary>
		/// Consumes all whitespace starting at <paramref name="index"/> and ending at the
		/// first non-whitespace character or the end of the buffer.
		/// </summary>
		/// <param name="index">The index at which to start skipping.</param>
		/// <returns>
		...
		void Slice(int start, ReadOnlySpan<TItem> slice);
